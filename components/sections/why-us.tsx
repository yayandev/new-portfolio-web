...
import { Code, Tag, Headphones } from "lucide-react";
...
export function WhyUs() {
  ...
  return (
    <section id="why-us" className="border-t border-stone-100 dark:border-stone-800/60">
          ...
          {WHY_US_ITEMS.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                className="relative p-6 sm:p-8 rounded-2xl border border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900/40"
              >
                <div className={`inline-flex p-3 rounded-xl bg-gradient-to-br ${item.gradient} text-white mb-5`}>
                  <Icon className="w-5 h-5" />
                </div>
                <h3 className="text-lg font-bold tracking-tight text-stone-900 dark:text-stone-100">
                  {t(dict, `why-us.${item.key}.title`)}
                </h3>
                <p className="mt-3 text-sm leading-relaxed text-stone-500 dark:text-stone-400">
                  {t(dict, `why-us.${item.key}.description`)}
                </p>
              </motion.div>
            );
          })}
